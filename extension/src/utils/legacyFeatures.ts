...
function findTextarea() {
    ...
    // 3. Shadow DOM (Light traversal)
    document.querySelectorAll("*").forEach(elem => {
        if (elem.shadowRoot) {
            const shadowTa = elem.shadowRoot.getElementById(TARGET_ID);
            if (shadowTa && shadowTa.tagName === "TEXTAREA") setupMonitoring(shadowTa as HTMLElement);
        }
    });
}
