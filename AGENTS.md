...
* **Pattern:** Analyze results survive page reload via `chrome.storage.local` (`dh_pending_analysis` + `dh_last_analysis`), with one-shot identity acknowledgments in `dh_seen_analysis`. The Service Worker owns result/pending writes; FAB only reads through `useAnalysisHydration` and writes the separate seen identity via `dismissPopover()`.
...
* **One-shot semantics:** New results persist `requestId`; legacy records use exact `caseNumber + timestamp`. `markSeen(identity)` writes only `dh_seen_analysis`; it never read-modify-writes `dh_last_analysis`. Hydration suppresses a result when its legacy `seen` flag is true or its identity matches `dh_seen_analysis`. A newer result naturally remains unseen because its identity differs and retains every field even when an older acknowledgment completes later. **DO NOT** bypass `popoverIsAnalyze.current` discrimination in the `ResultPopover` `onClose` handler — bookmark popovers share the same component and dismissing a bookmark must NOT acknowledge an analysis result.
