...
* **Tracking:** `self.current_session_id` holds the UUIDv5 session id used in reports and `--resume`. `self.current_case_id` tracks which case the session belongs to (used for smart-refresh comparison).
* **Resume:** The host tries `resume_session(name)` first (where `name` is the UUIDv5). If that fails, falls back to `create_session(session_id=name)`. Handles `AttributeError` gracefully if the SDK version doesn't support resume.
