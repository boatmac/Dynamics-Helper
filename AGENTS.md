...
* **One-shot semantics:** An immediate Analyze popover writes `seen=true` when dismissed; a rehydrated result is marked seen when FAB consumes it for display. A later mount does NOT reopen seen results. **DO NOT** bypass `popoverIsAnalyze.current` discrimination in the `ResultPopover` `onClose` handler — bookmark popovers share the same component and dismissing a bookmark must NOT mark an analysis result as seen.
