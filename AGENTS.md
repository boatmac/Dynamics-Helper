...
* **Options config persistence principle:**
  ...
  * **Persistence timing — Plan A (instant persistence, v2.0.70+):** There is **no Save button**. All Options fields persist immediately:
    ...
    * **Bookmark editor items (`dh_items`)**: every personal add/edit/delete/move/import/collapse and Reset intent increments one bookmark generation through `mutatePersonalItems`. Writes and Reset removal share one serialized storage queue; do not add raw personal `setItems` or direct `dh_items` mutation sites.
    * **Reset button**: still exists. Selects defaults and calls `persistPrefs(DEFAULT_PREFS)`; shared Service Worker cleanup may commit independently, but personal removal/default reload/success requires the captured bookmark generation to remain current through every async boundary. A newer personal mutation survives and produces a partial-reset warning.
...
* **Preference mirror actions:** Team sync, cache clear, manifest fetch where applicable, and Reset are user-intent actions, not storage-callback closures. Carry an unsettled action into compatible newer `dh_prefs` snapshots, settle it before dispatch so repair recursion cannot duplicate it, and cancel team actions when enabled/URL/team identity changes. Reset must survive unrelated post-reset edits and run once after a reset/default-derived snapshot commits.
...
* **Reset truth:** Reset requests/responses carry immutable default identity, request generation, and reset token. Local cleanup and success UI require a matching `committed` response; stale, failed, transport, or superseded callbacks preserve newer values and show incomplete Reset.
* **Native response metadata:** `normalizeNativeHostResponse` preserves success `data` unchanged. Error envelopes allowlist only safe caller metadata (`error_code`, string `errorKind`, finite numeric `httpStatus`) plus the fallback; never spread arbitrary Host fields into `NATIVE_MSG` responses.
* **Manifest retry truth:** Options keeps last successful manifest URL separate from tokenized in-flight URL. Only current identity-matching `committed`/`unchanged` callbacks mark success; every failure/stale/skipped/transport callback releases its own in-flight token, and an old URL callback cannot release or complete a newer URL.
