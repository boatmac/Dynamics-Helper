...
* **Session Names:** The host derives a stable session-name string from each 16-digit case ID via `_case_to_session_id()`, returning `dhco-<case>` (21 chars). This string is used as the `session_id` for both SDK `create_session()` and `resume_session()`, AND as the shell-CLI handle for `copilot --resume dhco-<case>`. Contract: DH-side extension of MyCasesKit B81 RFC § D1 (RFC form was `^(cc|co)-<case-num>$`; DH extends to `^(cc|co|dhco)-<case-num>$` — MyCasesKit repo update pending 2026-07-03). Length matters: **Microsoft Entra AAD's `client_session` OAuth parameter requires 20-50 chars**; the prior `co-<case>` (19 chars) failed with `AADSTS901001: Invalid request` and blocked MCP auth for every AAD-protected MCP tool. See `host/test_case_id.py::TestCaseToSessionId` for the regression guard including `test_length_satisfies_aad_minimum`.
* **Tracking:** `self.current_session_id` (Python field name retained for diff economy) holds the session-name string `dhco-<case>` used in reports and `--resume`. `self.current_case_id` tracks which case the session belongs to (used for smart-refresh comparison).
* **Resume:** The host tries `resume_session(name)` first (where `name` is `dhco-<case>`). If that fails, falls back to `create_session(session_id=name)`. Handles `AttributeError` gracefully if the SDK version doesn't support resume.
...
* **System Message Injection:** The session name is appended to the `system_message` content as a `## Session Info` section before session creation (labelled `Session Name: dhco-<case>`), so the AI can reference it during the conversation (e.g., for `context.md` frontmatter — MyCasesKit `session_name:` field per B81 RFC § D1, matcher extended to accept `dhco-` prefix).
