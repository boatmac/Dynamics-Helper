...
* **Options config persistence principle:**
  ...
  * **Persistence timing — Plan A (instant persistence, v2.0.70+):** There is **no Save button**. All Options fields persist immediately:
    ...
    * **Reset button**: still exists. Selects defaults and calls `persistPrefs(DEFAULT_PREFS)`; local bookmark/analysis cleanup completes only after the matching Service Worker Reset returns `committed`.
...
* **Preference mirror durability:** `dh_prefs` writes are single-flight and coalescing. Inspect `chrome.runtime.lastError`; no Host update or carried action may run until the latest intended snapshot commits. Failed writes remain visibly pending with unsettled actions for later user-driven retry.
