...
* **Options config persistence principle:**
  ...
  * **Persistence timing — Plan A (instant persistence, v2.0.70+):** There is **no Save button**. All Options fields persist immediately:
    ...
    * **Reset button**: still exists. Selects defaults and calls `persistPrefs(DEFAULT_PREFS)`. Reset is a two-phase transaction under one token: the default preference mirror and matching Host `update_config` must durably acknowledge before `RESET_EXTENSION_STATE` may run, and completion requires SW `syncStatus: 'committed'`. `config_saved: true` with refresh failure permits the SW phase but keeps the separate refresh warning. Host failure performs no SW destruction; SW failure retries only the SW phase. Personal removal/default reload/success additionally requires the captured bookmark generation to remain current through every async boundary, so newer edits survive and produce a partial-reset warning.
...
* **Preference mirror actions:** Team sync, cache clear, manifest fetch where applicable, and Reset are user-intent actions, not storage-callback closures. Carry an unsettled action into compatible newer `dh_prefs` snapshots, settle it before dispatch so repair recursion cannot duplicate it, and cancel team actions when enabled/URL/team identity changes. Reset must survive unrelated post-reset edits and run once after its reset/default-derived snapshot and matching tokenized Host update both commit.
...
* **Reset truth:** Reset requests carry immutable default identity, request generation, and one reset token through both the Host preference update and Service Worker cleanup. Never dispatch SW cleanup before the matching Host result is durably acknowledged (`success: true` or `config_saved: true`). Completion additionally requires matching SW `syncStatus: 'committed'`; stale, failed, transport, or superseded callbacks preserve newer values and show incomplete Reset. After Host commit, retry only the idempotent SW phase with the same token.
* **Native response metadata:** `normalizeNativeHostResponse` preserves success `data` unchanged. Error envelopes allowlist only safe caller metadata (`error_code`, string `errorKind`, finite numeric `httpStatus`) plus a string-only `error`/`message` fallback. Ignore objects, arrays, functions, and null without coercion, using the fixed `Native Host error` fallback; never spread arbitrary Host fields into `NATIVE_MSG` responses.
