...
Install Chrome or Edge and the supported Copilot CLI before qualification. Use
Windows local-disk redirection only to copy A and B onto the cloud PC. Copy both
ZIPs into `C:\DH-CloudPC`, then work only from that local directory. Never run
an archive, script, or installer from a redirected drive.

```powershell
if(-not(Test-Path -LiteralPath 'C:\DH-CloudPC')){New-Item -ItemType Directory -Path 'C:\DH-CloudPC' -ErrorAction Stop|Out-Null}
```
...
files appearing is not evidence of success.
...
integrity check, and disk/process baseline check again. Require Host and
...
PowerShell 7 windows for the two read-only watchers and the one-shot
interrupter. The watcher outputs are observational only; the one-shot
...
Start this process watcher in a second window. It classifies process mode but
never prints a complete command line. Stop it with Ctrl+C only after terminal
verification; its `finally` block unregisters the event subscription:
...
`original-runner-killed`, close every browser window within ten seconds, then
...
`$global:DhKilledRunnerPid`. It waits five seconds, proves the killed PID and all
browser/Host/runner executors are absent, strictly revalidates the same
post-activation transaction, and requires RunOnce to remain armed.
...
before kill and at zero executor; preserve the transaction and killed PID in
the same shell; and require a different-PID exact-path `--recover-active`
...
1. Re-establish and verify `plan-d-a` with the complete A installer.
2. Close all browser windows, run the installer guard, and use the complete B
   installer command to establish known-good B.
3. Restart the browser and require B versions, capability, verified integrity,
   Analyze PASS, and Options PASS.
4. Close all browser windows and create this harmless unexpected sentinel under
...
5. Run the installer guard, then run the exact same complete B installer command
   again and wait for its terminal success message.
6. Verify sentinel removal. Require `False`:

```powershell
Test-Path -LiteralPath "$env:LOCALAPPDATA\DynamicsHelper\_internal\dh-cloud-pc-sentinel.txt"
```

7. Restart the browser and require B Host/Extension versions, packaged verified
   integrity, Analyze PASS, and Options PASS. Confirm user-owned configuration
   and prompt files remain available, but do not copy their values, paths,
   contents, or hashes into the ledger.

Scenario 3 passes only if the sentinel is removed, the full B product verifies,
user-owned state is preserved, and both smokes pass. It intentionally does not
corrupt executable bytes or construct an unrecoverable mixed installation.
