...
all five Automated Gates to be `PASS`; `PENDING` and `Not recorded` fail closed.

```powershell
$ledger=(Resolve-Path -LiteralPath 'docs/plan-d-pragmatic-cloud-pc-results.md' -ErrorAction Stop).Path;$text=[IO.File]::ReadAllText($ledger);foreach($spec in @(@('A','2.0.74-beta.4'),@('B','2.0.76-beta.1'))){$pattern='(?m)^\| '+[regex]::Escape($spec[0])+' \| `?'+[regex]::Escape($spec[1])+'`? \| `?[0-9a-f]{7,40}`? \| `?[0-9a-f]{64}`? \| (?!(?:PENDING|Not recorded)\s*\|)[^|\r\n]+ \|$';if($text -cnotmatch $pattern){throw "Artifact $($spec[0]) identity is incomplete or malformed"}};foreach($gate in @('Host full suite','Extension full suite','Extension production build','Frozen Host build/probe','Static/reachability checks')){$pattern='(?m)^\| '+[regex]::Escape($gate)+' \| PASS \| (?!(?:Not run|Not recorded))[^|\r\n]+ \|$';if($text -cnotmatch $pattern){throw "Automated gate is not PASS: $gate"}};'Cloud PC qualification entry gate: PASS'
```

Also inspect those two ledger sections manually. Do not proceed if either
artifact identity is incomplete or any Automated Gate is not exactly `PASS`.
...
```powershell
$root='C:\DH-CloudPC';if(-not(Test-Path -LiteralPath $root -PathType Container)){New-Item -ItemType Directory -Path $root -ErrorAction Stop|Out-Null};$marker=Join-Path $root 'PLAN_D_EMPTY_CLOUD_PC.marker';$expected='PLAN_D_EFFECTIVELY_EMPTY_CLOUD_PC_V1';if(Test-Path -LiteralPath $marker){if(-not(Test-Path -LiteralPath $marker -PathType Leaf) -or [IO.File]::ReadAllText($marker) -cne $expected){throw 'Empty-cloud-PC marker is invalid'}}else{[IO.File]::WriteAllText($marker,$expected,[Text.UTF8Encoding]::new($false))};if([IO.File]::ReadAllText($marker) -cne $expected){throw 'Empty-cloud-PC marker verification failed'}
```

Every command that can invoke installer process termination or explicitly kill
a browser, Host, or runner begins by reading these exact marker bytes. A missing
or changed marker stops the command.
...
```powershell
$marker='C:\DH-CloudPC\PLAN_D_EMPTY_CLOUD_PC.marker';if(-not(Test-Path -LiteralPath $marker -PathType Leaf) -or [IO.File]::ReadAllText($marker) -cne 'PLAN_D_EFFECTIVELY_EMPTY_CLOUD_PC_V1'){throw 'Empty-cloud-PC marker is missing or invalid'};$b=@(Get-Process -Name chrome,msedge -ErrorAction SilentlyContinue);if($b.Count){throw 'Close all Chrome and Edge windows before running the installer'}
```
