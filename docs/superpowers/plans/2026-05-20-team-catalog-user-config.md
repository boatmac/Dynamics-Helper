...
- **SW telemetry silent failure** (pre-existing, exposed during this work): `@microsoft/applicationinsights-web` does not run cleanly in ServiceWorkerGlobalScope. SDK warns `Event handler of 'ai_cfgsync' event must be added on the initial evaluation of worker script` because `initTelemetry()` is async and the SDK tries to register listeners after the await on `chrome.storage.local.get`. Then `appInsights.context.user.id = stableUserId` throws `Cannot set properties of undefined (setting 'id')` because the SDK initialization aborted. Net effect: SW-side telemetry has been silently broken since commit 5ac5320 (~9 days). Fix likely requires restructuring `initTelemetry` to register all SDK event handlers synchronously at top level, or replacing the SDK with a direct REST POST to the App Insights ingestion endpoint. Not blocking - extension features work; only product metrics from background events are missing. Track separately.
