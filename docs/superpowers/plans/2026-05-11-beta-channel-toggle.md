...
- Discovered during the v2.0.70-beta publish: `release_helper.py:198-216` `publish_to_github()` builds the `gh release create` command as a single shell-string with `shell=True` AND a multi-line `--notes` argument. Trailing `--prerelease` flag is silently swallowed under that combination, so the release lands on GitHub with `isPrerelease: false` even when the caller passed `prerelease=True`. Worked around by `gh release edit v2.0.70-beta --prerelease` after the fact. Proper fix: convert `publish_to_github` to argv-list form (no `shell=True`), same pattern as the Phase 3 fix to `build_host()`. Cheap, no urgency unless we cut another prerelease before fixing.
