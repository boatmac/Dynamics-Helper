...
- **SDK ping-timestamp shim (host/dh_native_host.py:244-287, added in `b4bb6ab`)** — monkey-patches `copilot.client.PingResponse.from_dict` to tolerate ISO 8601 strings from CLI 1.0.46+. Delete the shim once `github-copilot-sdk` releases a version whose `PingResponse.from_dict` natively accepts ISO timestamps. Verify by running the host without the shim against the then-current CLI; if `client.start()` succeeds, the shim is redundant. **Verification 2026-05-21:** SDK 0.3.0 (latest on PyPI) still does `int(timestamp)` directly — `inspect.getsource(copilot.client.PingResponse.from_dict)` shows `return PingResponse(str(message), int(timestamp), int(protocolVersion))` with no ISO-string handling. Shim still mandatory. Re-run this verification after each SDK upgrade; the moment the source shows a `try/except` around `int(timestamp)` or an `isinstance(timestamp, str)` branch, the shim can be deleted in the same commit that bumps `requirements.txt`.
- **[CLOSED 2026-05-22]** **PageReader has no automated test** (extension has no unit test infra at all). The session-name reuse bug (caseNumber pollution by D365 task-ID + SKU concatenation) shipped through the test pyramid undetected. Setting up vitest / jest for extension/src and writing a regression test for `pageReader.idRegex` + `extractValueFromNeighbors` with the polluted-text fixture would catch the next variant. Tracked but not blocking; the host-side `_extract_case_id` strict regex catches the worst case (returns None instead of silent collision).

  **Resolution:** Vitest 3 + Testing Library 16 + jsdom landed per `docs/superpowers/specs/2026-05-22-extension-test-infrastructure-design.md` / `docs/superpowers/plans/2026-05-22-extension-test-infrastructure-plan.md`. Standalone `vitest.config.ts` (does NOT extend `vite.config.ts` — CRXJS plugin breaks jsdom), `pool:'forks'`, jsdom env. Chrome API mock at `extension/src/test/chromeMock.ts` exposes `installChromeMock()` / `resetChromeMock()` / `deferNextResponse(action)` / `seedStorage()` / `chromeMockSpies` covering both callback and Promise styles. Two regression suites delivered: (1) `pageReader.test.ts` exports + locks `ID_REGEX` accept/reject behaviour (the polluted-text variant this follow-up was originally written about); (2) `Options.test.tsx` — six tests T-Inv1…T-Inv6 mapping 1:1 to the six hydration-window invariants in `docs/superpowers/specs/2026-05-21-options-hydration-window-edits-design.md` § 4 + § 5, each break-and-fail verified per commit `673b5aa`. Scripts wired in `extension/package.json`: `npm test` (watch), `npm run test:run` (CI), `npm run test:coverage`. Docs updated: `AGENTS.md` § 2 (test commands + chromeMock API + 6-invariant + break-and-fail rule), `DEVELOPER_GUIDE.md` new "Extension Testing" section. The FAB rootPathOverride regression-test follow-up immediately below is now unblocked.
...
- **[CLOSED 2026-05-21]** **`useTranslation` storage path could collapse onto `usePrefs`** (added 2026-05-21, prefs-refactor follow-up). The `useTranslation` hook in `extension/src/utils/i18n.ts` read `dh_prefs.language` from storage with its own listener — duplicating exactly what `usePrefs()` does. Resolved in v2.0.70-beta.4: replaced the inline `useState<LanguageCode>` + `chrome.storage.onChanged` listener with `const { prefs } = usePrefs(); const storageLanguage = prefs.language ?? 'auto'`. Also dropped the `useState<'en'|'zh'>` cache for `resolvedLang` since `resolveLanguage()` is a pure function safe to call in render. Net: -24 lines, bundle -0.39 kB. Behaviour equivalent: storage→language path still async via usePrefs's own useEffect; ctxLanguage override path still wins over storage in Options tree. `setLanguage` removed from the hook's return value (grep confirmed zero callers).
- **[CLOSED 2026-05-22]** **FAB rootPathOverride regression test** (added 2026-05-21, prefs-refactor follow-up). The new `rootPathOverride` state in FAB.tsx + `effectivePrefs` merge is a documented exception to "all prefs come from the hook". A future refactor could accidentally collapse this back into the hook (e.g., trying to make the hook accept runtime overrides). Once extension test infra exists (follow-up #2), add a regression test that asserts: (a) FAB writes to `effectivePrefs.rootPath` are not persisted to `chrome.storage.local`, (b) Options-side rootPath changes are still observed by FAB unless an override is active, (c) clearing the override (setRootPathOverride(null)) restores Options' rootPath. This is a forward-looking guardrail; not actionable until follow-up #2 lands.

  **Resolution:** Commit `b8f5752`. Extracted the inline ternary at FAB.tsx:363-365 into a pure `mergeRootPathOverride(prefs, override)` helper in `utils/prefs.ts`, then wired FAB.tsx to call it (the import wiring is the part that makes the test a real guardrail — if a future refactor stops calling the helper, the test must lose protection of the running code). Added `extension/src/components/FAB.rootPathOverride.test.ts` with four tests mapping 1:1 to the three invariants in this follow-up plus a non-rootPath-leakage cross-cutting guard: F-Inv1 storage spies untouched (a), F-Inv2 override=null returns prefs by reference (b — also preserves useEffect dep-array identity), F-Inv3 clearing the override restores Options' rootPath (c), F-Inv4 override branch leaves non-rootPath fields intact. Strategy: pure-function unit tests rather than full FAB-component mount (FAB.tsx is ~1132 lines with MutationObserver / window listeners / chrome.runtime / setupContextMenu / usePrefs subscription — mounting in jsdom would require a wall of mocks that obscure the invariants). Break-and-fail verification table (per the convention from commit `673b5aa`): break #1 adds storage write inside merge → F-Inv1 fails; break #2 returns `{...prefs}` on null branch → F-Inv2 + F-Inv3 fail (identity-reference); break #3 ignores override → F-Inv1 + F-Inv3 fail (rootPath equality). 21/21 green, build clean.
...
- **[CLOSED 2026-05-21]** **Local Python env has wrong SDK version installed** (added 2026-05-21, observed while running DoD test step for follow-up #10). `host/requirements.txt` pins `github-copilot-sdk==0.3.0` but the active Python 3.13 site-packages had 0.2.0 installed. Symptom: `python -m unittest discover host` produced 1 failure + 9 errors all of the form `ImportError: cannot import name 'PreToolUseHookOutput' / 'PermissionRequestResultKind' from 'copilot.session'` or `'PermissionRequestResult' from 'copilot.generated.rpc'` (these symbols exist in 0.3.0 only). Verified pre-existing on commit `4071206` (before any follow-up #10 code change). Not a wire-drift incident per AGENTS.md § 9.5 — the upstream package is fine, the local venv just drifted (likely from a `dev_switch.py` flip or an `--upgrade` that downgraded back to 0.2.0). **Resolution:** ran `pip install -r host/requirements.txt --upgrade` (2026-05-21), SDK back to 0.3.0, `python -m unittest discover host` now reports 50/50 OK (the previously-failing imports were masking 26 additional tests that the unittest loader silently skipped). Next time host tests fail on import errors mentioning `copilot.*`, check `pip show github-copilot-sdk` first before assuming a real SDK wire-drift.
- **[CLOSED 2026-05-22]** **Options edits during hydration window silently lost** (added 2026-05-22, prefs-refactor follow-up; surfaced while diagnosing why a quick language switch right after opening Options didn't persist). Root cause: the coarse `prefsHydratedRef` guard in `persistPrefs` early-returned for the entire ~4 s window between Options mount and the host `get_config` response. The fast user click never reached `chrome.storage.local.set`, so cross-tree subscribers (`PrefsLanguageProvider`, `FAB`) never saw the edit, and the eventual hydration merge then overwrote the user's value with host's pre-edit state. Bug affected every dh_prefs field, not just language; language was the visible canary because of the cross-tree i18n context subscription. **Resolution:** split `persistPrefs` into three segments — `chrome.storage.local.set` runs unconditionally (storage is a safe mirror; this is what unblocks cross-tree `usePrefs` subscribers immediately), host RPC + manifest fetch stay gated on `prefsHydratedRef` (preserves the original guard's purpose: don't send DEFAULT_PREFS-empty values to host and clobber `config.json`/`user_prompt.md`). Added a `userTouchedFieldsRef: Set<keyof Preferences>` — `updatePref`, `handlePrefChange`, and the 6 Pattern-C inline onChanges mark the relevant key touched at edit time; the host hydration merge skips touched keys per-field; a catch-up RPC at hydration COMPLETE (success branch + non-success branch + `chrome.runtime.lastError` branch) pushes window-edits to host using `buildHostConfigPayload(mergedPrefs)`. `handleReset` marks all keys touched so a late hydration response cannot un-reset the user. Manual smoke per spec § 5: #1 post-hydration language switch ✅, #2 window-edit language switch persisted in `config.json` ✅ (verified by inspecting `extension_preferences.language` after the catch-up RPC's progress→success pair on requestId `b91d...`), #4 window-edit Reset survives late hydration ✅. Spec: `docs/superpowers/specs/2026-05-21-options-hydration-window-edits-design.md`. Commits: `0515ea8` (spec) + `957754e` (fix). Net diff +180/-54 (~+42 net code lines, ~+95 comment lines documenting the three-segment invariant). The bigger `PrefsContext` refactor in `docs/superpowers/plans/2026-05-21-prefs-context-refactor-plan.md` would have prevented this class of bug entirely, but is much larger scope; this tactical fix unblocks users now without precluding the strategic refactor.
- **[CLOSED 2026-05-22]** **Service Worker telemetry init throws `Cannot set properties of undefined (setting 'id')`** (added 2026-05-22, observed during hydration-window-edits smoke test). Console at SW startup shows `[DH-SW] Failed to initialize Telemetry in Background: TypeError: Cannot set properties of undefined (setting 'id') at D (serviceWorker.ts-DUuDcthh.js:1:1535)`. The minified function `D` is the telemetry initializer that sets `user.id` on the App Insights context. Most likely cause: the user identity UUID read from `chrome.storage.local` returns `undefined` (first-run or storage cleared) and the init code dereferences `context.user.id = ...` without initializing `context.user` first. Per AGENTS.md § 3 "Telemetry — User Identity: Stable anonymous UUID is generated via `chrome.storage.local` in `serviceWorker.ts`", the generator must be called before context.user.id assignment. Failure is non-fatal (catch reports it) but means background-context telemetry is dead until next SW restart. Pre-existing, not a regression from the hydration-window fix.

  **Investigation update:** the actual root cause was NOT a missing UUID. The `stableUserId` resolution at `serviceWorker.ts:30-35` always produces a non-empty string (reads `chrome.storage.local.get("telemetryUserId")`, falls back to `crypto.randomUUID()` + write). The TypeError fires at line 49 (`appInsights.context.user.id = stableUserId`) because in a service worker context the App Insights Properties plugin can fail to initialize the `user` sub-context — it relies on `document`/cookies which SWs lack — leaving `appInsights.context` or `appInsights.context.user` itself as `undefined`. Confirmed by minified bundle decode: col-1535 of `serviceWorker.ts-DUuDcthh.js` is literally `s.context.user.id=c`, no UUID-resolution step in between.

  **Resolution:** Commit `b89bfb8`. Wrap the direct property assignments in `if (appInsights.context && appInsights.context.user) { ... }`. When the guard fails, log a warning and fall through past the assignment — this ensures `addTelemetryInitializer` in step 4 still registers and stamps every item with `userId` + `extensionVersion` as custom dimensions (`item.data.userId`). App Insights backend reads userId from custom dimensions when the schema-level field is absent, so data completeness is preserved. Build green (new SW bundle hash `CiQebvcW`), `npm run test:run` 17/17 green. Automated unit testing the SW startup path is blocked on Vitest not supporting `ServiceWorkerGlobalScope` directly; the fix is a 4-line defensive guard around a 1-line assignment that has no other code paths into it, so the cost/benefit of adding a SW test harness was deferred.
- **[NON-BUG 2026-05-21]** **Two `python.exe` processes when host is running** (added 2026-05-21, observed during race-test session for `prefsHydratedRef` guard verification). `Get-CimInstance Win32_Process -Filter "Name = 'python.exe'"` consistently shows TWO entries with identical CommandLine when the host is alive (e.g., PID 19484 + 22252, then PID 29196 + 10532 on next session). Both reference `host\venv\Scripts\python.exe -u host\dh_native_host.py`. Killing the lower PID (parent) cascades the higher PID (child) automatically — `Stop-Process` reports the child as already gone. **Investigation:** this is **not** a Chrome double-spawn race and not a `multiprocessing` / `subprocess.Popen` call in `dh_native_host.py` (grep confirms only `CopilotClient(SubprocessConfig)` calls, which spawn `copilot.cmd` — not `python.exe`). It is the standard **Python venv launcher behavior on Windows**: `venv\Scripts\python.exe` is a 255 KB launcher stub that spawns the base interpreter at `C:\Users\zhaobo\AppData\Local\Programs\Python\Python313\python.exe` and stays alive as the parent process so signal forwarding / job-object semantics work. Both processes show the venv path in their CommandLine because the child inherits the parent's command line — `sys.executable` still resolves to the venv path inside the actual running interpreter. **No fix needed.** Recorded so a future agent (or future me) doesn't mistake this for a spawn race. The only operational gotcha: when scripting host kills, target only the parent (lowest PID) — killing the child first leaves the parent zombie. The current `Stop-Process -Force` pattern in `dev_switch.py` and ad-hoc test scripts already handles this correctly because it iterates over all matching processes and Windows propagates SIGTERM-equivalent through the venv launcher.
