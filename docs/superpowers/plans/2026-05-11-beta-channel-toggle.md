...
- **`release_helper.py` hardcodes release notes** (added 2026-05-21, beta.3 follow-up). `publish_to_github()` at `release_helper.py:261` builds the GitHub release body from a 4-line f-string template (`"Release {tag}\n\n## Installation\n..."`). For beta.3 we had to write the real release notes to a separate markdown file and call `gh release create ... --notes-file` manually — bypassing helper's `--publish` flag entirely. Two related papercuts:
