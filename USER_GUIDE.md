...
**Built-in defaults:** New browser profiles start with public Dynamics Helper documentation, release, and issue-reporting links. An upgrade does not replace a saved personal bookmark menu. Missing or non-array personal bookmark storage falls back to the shipped defaults; an explicitly saved empty menu remains empty. Clearing browser storage or clicking **Reset** loads the currently shipped public defaults again.
