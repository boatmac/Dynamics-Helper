...
CURRENT TASK: exact profile-delta source rule and pure validation completed24/24.
Corrected PowerShell harness has not been rerun. Earlier single invocation:
workflow assertions passed but profile-emptiness failed (empty AppData/Roaming).
One approved minimal baseline reproduced that exact empty-directory shape without
loading installer. No harness retry. Prior pure-Python runner validation7/7.
Prior checker-only validation is complete (12/12). The approved synthetic execution
used plain Python child scripts only; no process-tier product validation occurred.
General product tests, PowerShell, payload decoding, cleanup, builds, installs and
Git writes remain paused after the company security alert. SDK work remains paused.
Preserve all original private scripts/logs and follow docs/test-safety.md.

Stable v2.0.76 is published. Read-only SDK1.0.13 compatibility research completed
without changing the1.0.5 runtime. User-approved TEMP1.0.13 installation/mock
qualification is complete;25/27 passed with findings recorded in SDK assessment.
...
Latest SDK stage: user approved a new disposable1.0.13 environment and mocked
qualification only. Created localTemp dh-sdk-1013-qualification-20260909, verified
official wheel hash941dd5b55cf32ba55c73c651052a4a52b259b470c68bf6a6ac3d240c235402c9,
retained13transitive pins, hash-required offline install/pipcheck passed. Project
venv remains1.0.5, tracked productfiles unchanged, no runtime/CLI/model/build/cloud
or security mutation.27cases executed25pass2fail; two passes are known-gap
observations, not safety approvals. No tasks/processes survived.

Independent review confirmed synthetic successfalse options-patch response is
valid and ignored bySDK; DH isolation guarantee needs negative-ack failure handling
before adopting. Managed-required request currently approved byDH mockhandler;
proposed concrete denial tested TEMPonly. Cleanupfailure fixture had no reader/
CLI, so it is inconclusive for production leaks and does not justify cleanupshim.
See docs/sdk-1.0.13-upgrade-assessment.md for exact scope/evidence. These reporting
edits are uncommitted; no product pin or AGENT permissionrule changed.

## Next Single Action

Remain in the LOCAL checkout
`C:\MyWorkbench\Repository\Dynamics-Helper-prompt-scope-spec`. Report exact
directory-delta validation24/24 (9Python syntaxchecks) and pending corrected
PowerShell run. Preserve historical failed/baseline records, no reinterpretation.
One corrected plain success-harness run requires explicit process authorization;
no product suite, SDK, inventory approval, commit/push/release now.

The approved pure-Python step added scripts/test_profile_state.py and12inert
test_profile_state groups, sharing exactsnapshot validation with both future
installerharness call sites. Supervisor schema2 distinguishes profiles_empty
from profile_delta_allowed and hashes shared helper; prelaunch remains empty.
Reject files/extradirs/aliases/links/reparse/unknownrootchanges; cap64entries/root,
do not traverseunknown dirs, errorspropagate. tests/validate_test_safety.py ran
once basePython-I-B-S,9syntaxfiles and24/24 tests passed in0.112s. Mockfilesystem
only, noPS/Host/SDK/process/filecreation from tests, original evidence untouched.
Staticreview no blockers; runner inventory remains null/unapproved.

User-approved minimal startup comparison completed once via plain
tests/validate_powershell_startup.py -> powershell_startup_baseline.ps1 -File,
same flags/environmentshape. Baseline no installer imports/operations; PID19628
exit0,0.257s,29stdoutbytes,0stderr. Profiles after: only USERPROFILE/AppData and
USERPROFILE/AppData/Roaming empty directories; other5roots empty. Source hashes
unchanged, childPID absent afterward. Evidence Temp dh-powershell-baseline-jtiqk77u
retained. Supports currentenvironment PowerShell startup baseline, not universal
sideeffectabsence or malicious/falsepositive conclusion. Original incident and
previous harness records untouched. No secondinvocation or auto-cleanup.

User approved ONE plain success invocation. New reviewed supervisor
tests/validate_installer_harness.py launched fixedPS-File success with6isolateddirs,
20sec budget/1MiBperstream. PID66656 exit0 in1.280s, workflow assertions pass and
27invaliddeps inferred complete. Source hashes unchanged, stderr empty, readers
closed; subsequent PIDcheck absent. Overallpassed=false because USERPROFILE gained
empty AppData/Roaming, confirmed by readonly directory inspection. Exact origin
unproven; no global containment/Defender-no-alert claim. Other5dirs empty. Evidence
Temp dh-installer-validation-7bfjev70 retained unmodified. No secondPSinvocation,
actualHost/SDK/product installation/securitychange, or original evidence access.

Latest review confirms nine explicit fake operations and definitions-only
dot-source of installer_core; no default factory or real native/file operations
on that path. Python local imports reviewed without execution: install_integrity,
package_manifest, product_info, test_update_support; no Host/SDK import chain.
Current manifest remains pending, not ready for repository runner execution.
Removed unnecessary harness string interpolation and added partial-output/timecode
retention on Python harness timeout. These edits were source-only; no syntax
parser, hash, test or PowerShell ran. Original security evidence remains untouched.

The preceding preparation used source edits only; the subsequent authorized
execution below is the latest validation evidence, not a source-only claim.
The proposed supervisor uses seven serial direct Python children and four
runner-owned synthetic workers, all with `-I -B -S`, retaining new evidence only
under `C:\Users\zhaobo\AppData\Local\Temp\opencode`. Cases cover production gate
rejection, pass/fail progress, wrong manifest/selection hashes, timeout and output
budget. Fixture copies are byte-checked; temporary manifests contain data only,
one fixed fixture each and zero exceptions. The production checker/manifest stay
unchanged and incomplete/null. No Host/SDK/PowerShell imports or execution are
part of this validation. Preserve prior evidence and dirty files; no Git writes.

Pre-execution review corrections are source-only: runner and supervisor reject
missing/non-absolute/nonexistent base executable metadata, executable/base identity
mismatch, and differing prefix/base_prefix before gate/spawn. Start directly with
`C:\Users\zhaobo\AppData\Local\Programs\Python\Python313\python.exe`, never a
venv redirector, `py` launcher or PATH alias. Both descendant levels use that same
resolved executable with `-I -B -S`; the passing fixture checks base identity too.
These identity checks do not prove process ownership or descendant containment.
The noisy case requires runner rejection and the exact output cap, not worker
interruption: its finite writer may finish with exit 0 before cap detection.
Timeout still requires nonzero worker exit and no finished event. Seven cases
remain planned; no execution or hash computation occurred during these edits.

Execution completed using basePython -I-B-S tests/validate_safe_runner.py once.
New evidence Temp dh-runner-validation-wqarzrmx records7/7 PASS expected outcomes,
source_unchanged=true/evidence_retained=true. Production review gate blocked;
purefixture passed2assertions; intentionalfailure returned1; manifest/selection
wronghash rejected beforefixtures; timeout3seconds terminated only ownedworker;
stdoutlimit retained exactly8MiB and rejectedrun. All11recorded Python PIDs absent
on targeted followup. No PowerShell/Host/SDK/network/productinstall ran or prior
incidentfiles were accessed. Real manifest remains incomplete/null, no inventory
exceptions approved. This validates finite synthetic behavior, not OS sandboxing
or arbitrary descendant cleanup. No new SDK work or Git write authorized.

## Test-Safety Incident And Source Remediation

The user received company alert Possible Metasploit activity covering2026-09-08
10:25:32-10:27:09 UTC, naming b4final-host.py and compressed Base64/GZip PowerShell
Invoke-Expression loaders. Read-only records place the AI-initiated beta4 test run
around that window; source generator was InstallerSafetyTests._run_installer.
It launched actual PowerShell scenario processes, not merely inert string tests.
No Metasploit/Meterpreter installation or invocation was directed by this session.
Command shape/time correlation is established; exact alert payload bytes and full
endpoint effects were not independently verified. Do not call it a proven false
positive or claim mocks/temporary roots were an OS sandbox. Do not copy opaque
alert payloads into repository fixtures or execute/decode them to test attribution.

User approved source/rule remediation only. Changed installer_core.ps1 in place
to definitions-only dot-source plus explicit required Ops workflow/real-adapter
factory. This avoids adding package files that old exact-inventory validators
would reject. New checked-in plain tests/harnesses/installer_safety.ps1 uses only
recording operations for15 scenarios and invalid-dependency rejection. Python
installer tests now invoke -File with explicit opt-in and reduced isolated env,
parse structured events, and retain scenario evidence even when tests pass.
Installer ordering/user-data/protection contracts remain intended but UNTESTED.

Added scripts/check_test_safety.py, tests/test-safety-manifest.json and inert
tests/test_test_safety.py. Forbidden execution cannot be exempted; unresolved
calls and changed/pending reviews block. Inventory is intentionally incomplete,
hashes null, so no green gate is claimed. Added scripts/run_safe_tests.py requiring
gate approval before imports in parent/worker, explicit process opt-in, isolated
environment, bounded progress/output and retained evidence. These controls are
accident prevention, not adversarial confinement; descendants may outlive worker.

Two bounded static reviews found one suppressible-forbidden-sink issue, fixed
in source; runner review found no further definite blocker. No tests, parsers,
interpreters, hashes, real subprocesses, network, SDK calls or builds ran in the
remediation phase. No original Temp evidence was altered/deleted. docs/test-safety.md,
AGENTS.md and DEVELOPER_GUIDE.md reflect the new execution boundary. Published
stable package remains unchanged; source changes are not yet runnable-qualified.
