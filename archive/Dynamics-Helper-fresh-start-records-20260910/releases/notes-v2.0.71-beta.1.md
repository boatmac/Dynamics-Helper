...
**Architecture:** the Service Worker owns all storage writes — FAB only reads. Pure helpers (`extension/src/utils/analysisStore.ts`, `extension/src/background/analyzeBridge.ts`, `extension/src/hooks/useAnalysisHydration.ts`) keep the persistence logic unit-testable without a real Chrome port. Spec: `docs/specs/2026-06-03-analysis-result-persistence-design.md` (9 invariants P-I1..P-I4 + R-I1..R-I5). Critical pattern documented in AGENTS.md § 4.9.
